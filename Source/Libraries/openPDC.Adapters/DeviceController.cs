    ...
    public class DeviceController : ApiController
    {
        ...
        public IHttpActionResult UpsertDevices(IReadOnlyList<Device> devices)
        {
            try
            {
                ...
                var response = CommonController.ExecuteWithRetry(() => UpsertDevicesRecord(devices), nameof(UpsertDevices));
        ...
        public record UpsertDeviceResponse
        {
        ...
        public record UpsertDeviceResponseDetail
        {
        ...
        /// batch in a single database round-trip for the existing-device lookup, which keeps large
        /// batches (hundreds of PMUs) fast compared to querying once per device. A failure on one
        /// device is caught and recorded individually so it does not abort the rest of the batch.
        /// </summary>
        private UpsertDeviceResponse UpsertDevicesRecord(IReadOnlyList<Device> devices)
        {
            Log.Publish(MessageLevel.Info, nameof(UpsertDevicesRecord), $"Upserting {devices.Count} device record(s)");
            ...
            foreach (var device in devices)
            {
                ...
                try
                {
                    ...
                    if (!deviceExists)
                    {
                        ...
                        Log.Publish(MessageLevel.Info, nameof(UpsertDevicesRecord), "Device added successfully", details: device.Acronym);
                    }
                    else
                    {
                        var restriction = new RecordRestriction("Acronym = {0}", deviceInDatabase.Acronym);
                        ...
                        Log.Publish(MessageLevel.Info, nameof(UpsertDevicesRecord), "Device updated successfully", details: deviceInDatabase.Acronym);
                    }
                ...
                catch (Exception ex)
                {
                    ...
                    Log.Publish(MessageLevel.Error, nameof(UpsertDevicesRecord),
